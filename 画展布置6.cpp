...
/*
题解：
如果使用双重for循环，时间肯定是不够的，根据题目给出的公式，如果取除绝对值符号，其实l的值就是末尾减去开头的值，那么该如何去除绝对值符号呢？
答案是排序，这样就不会有负值了，这样只要一层for循环就可以解决问题。

需要注意的是，平方和任意超出int存储范围，需要long long 来存储
*/
...
int n,m;
long long ans=LONG_LONG_MAX,temp;
int q[100010];
...
int main(){
  ...
  for(int i=1;i+m-1<=n;i++){
    temp = (long long)q[i+m-1] * q[i+m-1] - (long long)q[i] * q[i];//注意要先转成long long 再乘
